...
{
	char buffer[1024];
    if (XBMC->GetSetting("ServerURL", &buffer)) 
    {
		g_ServerURL = buffer;
    }
	float fZone;
	if (XBMC->GetSetting("TimeZone", &fZone))
	{
		g_iEPGTimeShift = (int)((fZone) * 3600.0); // hours to seconds
	}
}
...
{
  static const char *strBackendName = "P2pProxy PVR Add-on";
...
{
  snprintf(signalStatus.strAdapterName, sizeof(signalStatus.strAdapterName), "P2pProxy Adapter 1");
...
int GetRecordingsAmount(void) { 
	if (m_data)
		return m_data->GetRecordingAmount();
	return -1; 
}
